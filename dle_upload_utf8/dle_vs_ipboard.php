...
class ipb_member
{
	...
	public function login(array $member)
	{
			...
			)
		{
			...
			if (!empty($this->member['member_id']))
			{
				...
				$_SESSION['processing2FA'] = array( 'memberId' => $i_member->member_id, 'anonymous' => 0, 'remember' => 1, 'destination' => (string) \IPS\Login::getDestination(), 'handler' => NULL );
				\IPS\Session::i()->setMember( $i_member );
				$device->updateAfterAuthentication( TRUE );
				\IPS\Member::memberSync( 'onLogin', array( \IPS\Login::getDestination() ) );
			}
		} elseif (!empty($row['id'])) {
	...
	public function UpdateProfile($user_name, $email, $password, $info, $admin = false)
	{
		...
		if ($admin)
		{
			$user_dle = $this->db->super_query("SELECT * FROM " . USERPREFIX . "_users WHERE name='" . $user_name."'");
